...
class State:
	...
	def relax(self, father, astar):
		if self.father == father:
			...
			self.f = self.g + self.h
			astar.open_count += 1
			heapq.heappush(astar.open, (self.f * 100 + self.h + astar.open_count * 0.000001, self))
			for i in self.child:
				i.relax(self)
		else:
			if self.father != self:
...
class Astar:
	def __init__(self, initial, goal, h_mode):
		...
		self.open = [(self.initial.f*100+self.initial.h+self.open_count*0.000001, self.initial)]
		self.close = []
	...
	def get_best_path(self):
		...
		while now != self.initial:
			#print(len(best_path)+1,now.get_graph())
